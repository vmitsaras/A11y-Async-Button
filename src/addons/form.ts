...
export type AsyncButtonFormState =
  | "idle"
  | "loading"
  | "success"
  | "error"
...
export interface AsyncButtonFormContext {
  ...
  setStatus(message: string, state?: AsyncButtonFormState): void;
}

export interface AsyncButtonFormEventDetail {
  instance: AsyncButtonFormInstance;
  state: AsyncButtonFormState;
  error?: unknown;
}

export interface AsyncButtonFormInstance {
  readonly form: HTMLFormElement;
  readonly button: HTMLButtonElement;
  readonly asyncButton: AsyncButtonInstance;
  readonly statusElement: HTMLElement | null;
  setStatus(message: string | null, state?: AsyncButtonFormState): void;
  destroy(): void;
}

export interface AsyncButtonFormOptions
  ...
  invalidStatusText?: string | null;
  onSubmit: (context: AsyncButtonFormContext) => Promise<unknown> | unknown;
}

interface NormalizedAsyncButtonFormOptions {
  button: string | HTMLButtonElement | null;
  status: string | HTMLElement | null;
  validate: boolean;
  idleStatusText: string | null;
...
} satisfies Omit<NormalizedAsyncButtonFormOptions, "onSubmit">);

interface StatusOriginalAttributes {
  text: string;
  state: string | null;
  role: string | null;
...
function toSafeTarget<TElement extends HTMLElement>(
  value: unknown,
  fallback: string | TElement | null
): string | TElement | null {
  if (typeof value === "string") {
...
function definedOptions(options: AsyncButtonFormOptions): Record<string, unknown> {
  const filteredOptions: Record<string, unknown> = {};

  for (const key of Object.keys(options) as Array<keyof AsyncButtonFormOptions>) {
...
function getAsyncButtonOptions(
  options: AsyncButtonFormOptions
): AsyncButtonOptions {
...
function readDataOptions(form: HTMLFormElement): Record<string, unknown> {
  return {
...
): NormalizedAsyncButtonFormOptions {
  const onSubmit = toSafeCallback<NormalizedAsyncButtonFormOptions["onSubmit"]>(
    rawOptions.onSubmit
  );

  if (!onSubmit) {
    throw new Error("createAsyncButtonForm requires an onSubmit handler.");
  }

  return {
    button: toSafeTarget<HTMLButtonElement>(
      rawOptions.button,
      DEFAULT_FORM_OPTIONS.button
    ),
    status: toSafeTarget<HTMLElement>(
      rawOptions.status,
      DEFAULT_FORM_OPTIONS.status
    ),
    validate: toSafeBoolean(
      rawOptions.validate,
      DEFAULT_FORM_OPTIONS.validate
    ),
    idleStatusText: toNullableString(
      rawOptions.idleStatusText,
      DEFAULT_FORM_OPTIONS.idleStatusText
    ),
    loadingStatusText: toNullableString(
      rawOptions.loadingStatusText,
      DEFAULT_FORM_OPTIONS.loadingStatusText
    ),
    successStatusText: toNullableString(
      rawOptions.successStatusText,
      DEFAULT_FORM_OPTIONS.successStatusText
    ),
    errorStatusText: toNullableString(
      rawOptions.errorStatusText,
      DEFAULT_FORM_OPTIONS.errorStatusText
    ),
    invalidStatusText: toNullableString(
      rawOptions.invalidStatusText,
      DEFAULT_FORM_OPTIONS.invalidStatusText
    ),
    onSubmit
  };
}

export class A11yAsyncButtonForm implements AsyncButtonFormInstance {
  ...
  public readonly statusElement!: HTMLElement | null;

  private readonly options!: NormalizedAsyncButtonFormOptions;
  private readonly originalStatus!: StatusOriginalAttributes | null;
  private readonly handleSubmit!: (event: SubmitEvent) => void;
  private isDestroyed = false;
  private isSubmitting = false;

  constructor(form: HTMLFormElement, options: AsyncButtonFormOptions) {
    if (!isFormElement(form)) {
      throw new Error("A11yAsyncButtonForm requires a <form> element.");
    }
    ...
    this.asyncButton = createAsyncButton(
      this.button,
      getAsyncButtonOptions(options)
    );
  ...
  setStatus(message: string | null, state: AsyncButtonFormState = "idle"): void {
  ...
  private resolveButton(): HTMLButtonElement {
    ...
    if (isButtonElement(configuredButton)) {
    ...
    throw new Error(
      "A11yAsyncButtonForm requires a submit button in the form or a button option."
    );
  }

  private resolveStatusElement(): HTMLElement | null {
    const configuredStatus = this.options.status;

    if (typeof configuredStatus === "string") {
      const formStatus = querySelectorSafely(this.form, configuredStatus);
      const documentStatus =
        formStatus ??
        (typeof document === "undefined"
          ? null
          : querySelectorSafely(document, configuredStatus));

      return isHTMLElement(documentStatus) ? documentStatus : null;
    }

    if (isHTMLElement(configuredStatus)) {
  ...
  private prepareStatusElement(): void {
    ...
    if (!this.statusElement.hasAttribute(ATTRIBUTES.role)) {
      this.statusElement.setAttribute(ATTRIBUTES.role, "status");
    }

    if (!this.statusElement.hasAttribute(ATTRIBUTES.ariaLive)) {
      this.statusElement.setAttribute(ATTRIBUTES.ariaLive, "polite");
    }

    if (!this.statusElement.hasAttribute(ATTRIBUTES.ariaAtomic)) {
      this.statusElement.setAttribute(ATTRIBUTES.ariaAtomic, "true");
    }
  }
  ...
  private onSubmit(event: SubmitEvent): void {
    ...
    if (
      this.isDestroyed ||
      this.isSubmitting ||
      this.asyncButton.getState() !== "idle"
    ) {
      return;
    }
    ...
    void this.runSubmit(event);
  }

  private reportInvalid(event: SubmitEvent): void {
    this.setStatus(this.options.invalidStatusText, "invalid");

    if (typeof this.form.reportValidity === "function") {
  ...
  private async runSubmit(event: SubmitEvent): Promise<void> {
    ...
    try {
      ...
      this.dispatchLifecycleEvent(EVENTS.error, { state: "error", error });
    } finally {
      this.isSubmitting = false;
    }
  }

  private createContext(event: SubmitEvent): AsyncButtonFormContext {
    const submitter =
      "submitter" in event && isHTMLElement(event.submitter)
        ? event.submitter
        : null;
  ...
  private dispatchLifecycleEvent(
    type: string,
    detail: Omit<AsyncButtonFormEventDetail, "instance"> & { error?: unknown }
  ): void {
    this.form.dispatchEvent(
      new CustomEvent<AsyncButtonFormEventDetail>(type, {
        bubbles: true,
        detail: {
          instance: this,
          ...detail
...
export function createAsyncButtonForm(
  form: HTMLFormElement,
  options: AsyncButtonFormOptions
): AsyncButtonFormInstance {
  return new A11yAsyncButtonForm(form, options);
}

export function initAsyncButtonForms(
  options: AsyncButtonFormOptions,
  root?: ParentNode
): AsyncButtonFormInstance[] {
  ...
  const rootElement = isFormElement(scope) && scope.matches(SELECTORS.root)
    ? [scope]
    : [];

  return [
    ...rootElement,
    ...Array.from(scope.querySelectorAll(SELECTORS.root)).filter(isFormElement)
  ].map((form) => createAsyncButtonForm(form, options));
}
