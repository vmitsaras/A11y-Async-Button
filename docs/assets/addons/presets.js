...
//#region src/addons/presets.ts
const PRESET_COMPONENT_NAME = "a11y-async-button-preset";
const DEFAULT_PRESET_NAME = "save";
const SELECTORS = Object.freeze({ root: "[data-a11y-async-preset]" });
...
const asyncButtonPresets = Object.freeze({
	save: Object.freeze({
		name: "save",
...
function pickAsyncButtonOptions(options) {
...
function readDataOptions(button) {
	return {
		...
		liveRegionPoliteness: getAttributeValue(button, ATTRIBUTES.liveRegionPoliteness)
	};
}
function mergePresetMaps(presets) {
	const customPresets = {};
	if (isRecord(presets)) {
		for (const [name, preset] of Object.entries(presets)) if (isPresetDefinition(preset)) customPresets[name] = preset;
	}
	return {
		...asyncButtonPresets,
		...customPresets
...
function normalizeOptions(rawOptions) {
	...
	const presetName = isPresetDefinition(configuredPreset) ? toSafeString(configuredPreset.name, "custom") : toSafeString(configuredPreset, DEFAULT_PRESET_NAME);
	const preset = isPresetDefinition(configuredPreset) ? configuredPreset : presetMap[presetName];
	if (!preset) throw new Error(`Unknown async button preset: "${presetName}".`);
	return {
		presetName,
		preset,
		asyncButtonOptions: {
...
function getAsyncButtonPresetOptions(preset, options = {}) {
	return normalizeOptions({
		...definedOptions(options),
		preset
	}).asyncButtonOptions;
}
var A11yAsyncButtonPreset = class A11yAsyncButtonPreset {
	...
	constructor(button, options = {}) {
		if (!isButtonElement(button)) throw new Error("A11yAsyncButtonPreset requires a <button> element.");
...
function createAsyncButtonPreset(button, options = {}) {
	return new A11yAsyncButtonPreset(button, options);
}
function initAsyncButtonPresets(options = {}, root) {
	if (!root && typeof document === "undefined") return [];
	const scope = root ?? document;
	return [...isButtonElement(scope) && scope.matches(SELECTORS.root) ? [scope] : [], ...Array.from(scope.querySelectorAll(SELECTORS.root)).filter(isButtonElement)].map((button) => createAsyncButtonPreset(button, options));
}
//#endregion
export { A11yAsyncButtonPreset, asyncButtonPresets, createAsyncButtonPreset, getAsyncButtonPresetOptions, initAsyncButtonPresets };
